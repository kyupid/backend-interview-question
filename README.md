...
- SQL Mapper VS ORM
- Spring MVC 동작 방식 (타임리프가 뷰로 보낼떄까지)
- 스프링 내부 어디까지 가보았나 - 디버깅했나 혹은 추적해서들어갔나
...
<details>
  ...
  <p>추상클래스와 인터페이스는 둘다 인스턴스화 할 수 없고 추상화를 목적으로 사용되는 하나의 껍데기라고 할 수 있습니다.</p>
  <p>인터페이스는 추상메소드 밖에 선언하지 못하는 반면 추상클래스는 추상메소드 뿐만아니라 non-abstract 메소드도 선언할 수 있습니다.</p>
  
  <p>추상클래스를 상속받은 또다른 추상클래스는 부모추상클래스가 가지고 있는 추상메소드들을 꼭 모두 구현할 필요없이 자식클래스가 구현하도록 남길 수 있습니다.</p>
  <p>인터페이스를 구현하는 것은 인터페이스가 선언한 추상메소드들을 모두 구현해야하므로 이런 차이점때문에 추상클래스와 인터페이스는 사용용도가 달라질수있습니다.</p>
